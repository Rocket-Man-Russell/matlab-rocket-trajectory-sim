%% KINGSROCKsim // Finian Russell // Kingston Uni MEng Astro 2016/17
%% Final Script - Dev note: search %%%DEV:NB%%%
%  Copyright 2017 Finian P.G. Russell
%  Apache License 2.0 must be included if redistributing
%{
...
%
%%%
while reRunSim == true % un-comment this line when integrating scripts
...
elseif inputsFromFile == true
    ...
    if  sameInputs == false
        inputsFileList = importdata('Simulation Descriptors.txt');
        disp('The simulation input data files are as such:');
        disp(inputsFileList);
    ...
    try
        ...
        if  strcmp(ME.identifier,'MATLAB:textscan:handleErrorAndShowInfo')
            inputsFileID = fopen([inputsFileName,'.txt']);
            inputsData = textscan(inputsFileID, '%f %*[^\n]');
            inputsData = cell2mat(inputsData);
    ...
    numOfSteps = round(expFlightDur/timeStep);
...
[   sim_time,...
    ...
    sim_angularAccel,...
    sim_resultantFlow,...
    sim_resultantFlowAngle  ] = deal(zeros(1,round(expFlightDur/timeStep)));
...
[   sim_time,sim_thrust,sim_massFlow,sim_area,...
    ...
    sim_angleOfAttackRAD, sim_liftCoef, sim_windLift, sim_dragCoefI,...
    sim_COP, sim_COM, sim_MOI, sim_stability, sim_propellantMass,...
    sim_fuelMass, sim_pitchAngleRAD, sim_angleDiff, sim_angularVelo,...
    sim_angularAccel, sim_resultantFlow, sim_resultantFlowAngle ...
    ...
    ...
    sim_pitchAngleRAD,sim_angleDiff,sim_angularVelo,sim_angularAccel,...
    sim_resultantFlow,sim_resultantFlowAngle);
...
if  rocketFromFile == false
    ...
    fprintf('%s\n','Surface Finish Options','1: Totally smooth surface','2: Polished metal or wood','3: Natural sheet metal',...
            '4: Smooth matte paint finish','5: Standard paint finish')
    ...
    disp([  'From nose to nozzle, each part one-by-one, describe the structure',...
            'of your rocket using the key below: '])
    ...
    while keep_adding == true
        ...
        part_identity{1,part_count} = input([   'Add a rocket part using the above key,'...
                                                'entering a number from 1 to 4: ']);
        ...
        switch part_identity{1,part_count}
            ...
            case 1  % Nose cone
                ...
                if  part_identity{2,part_count} == noseShape.Bi_conic
                    ...
                    nose_baseRadius{2}  =(input('Enter a value for the frustum top radius: '))*10^-metricLength;
                    nose_length{1}      =(input('Enter a value for the nose cone length: '))*10^-metricLength;
                    nose_length{2}      =(input('Enter a value for the frustum length: '))*10^-metricLength;
                else
                    ...
                    nose_tipRadius      =(input('Enter a value for the nose tip radius (zero if pointed, non-zero if curved): '))*10^-metricLength;
    ...
    if  sameRocket == false
        ...
        while isempty(rocketName) == true
            try
                ...
                elseif  strcmp(ME.identifier,'MATLAB:textscan:handleErrorAndShowInfo')
                        rocketFileID = fopen([inputsFileName,'.txt']);
                        rocketData = textscan(rocketFileID, '%f %*[^\n]');
                        rocketData = cell2mat(rocketData);
...
disp([  'You have the option to enter the rocket''s wetted surface area manually,'...
        'or the program will estimate it from your design.'])
...
disp('As of alpha 1.02, you can now add launch lugs to your rocket design.')
...
if  wind_velocity ~= 0
    ...
    'Enter a value for the angle that the wind vector makes with your rocket at launch (0 or 180), in degrees: ',yawAngle,[0,180]);
...
for stage_count = 1:numOfStages
    fprintf('%s\n', 'The types of motor simulated are as such:',...
                    '1. Solid','2. Monopropellant','3. Bipropellant','4. Hybrid')
...
elseif (rocketFromFile == true) && (chuteDataStart > 0)
...
switch failureOption
    case 0 % select failure mode
        [failureMode] = changeInput(simRunNum,'type of failure mode to simulate',...
                        'Enter a value from the above key for the type of failure mode to simulate [1-8]: ',...
                        failureMode,[1,2,3,4,5,6,7,8]);
...
while engineCounter < numOfStages
    ...
    if  keep_adding == true
        ...
        engineCounter = engineCounter + 1;
            ...
            sameEngine = false;
        if  simRunNum > 1
            sameEngine =  max(strcmp((input(['Use the same engine file for stage ',...
                                            num2str(stageCounter),'? [Y/N]: '],'s')),yes));
        end
        ...
        if  sameEngine == false
            ...
            while isempty(engineFile) == true
                try
                    engineFile = input(['Type the file name for your stage ',...
                                        num2str(stageCounter),' engine file (with the extension): '],'s');
                    disp('Please wait...')
        ...
        thrustData{engineCounter} = [0,0;engine.data]; % fixed for standard engine files
...
numOfEngines = engineCounter;
    ...
    if  numOfEngines > 0
        ...
        makeGraph('Line',simRunNum,'Thrust Curve','Time (s)','Thrust (N)',...
                    thrustGraphData(:,1),thrustGraphData(:,2))
    end
...
%%
%
fprintf('%s\n','Now COP (Centre of Pressure) of the rocket will be calculated.')

[bodyTubeNUM,transitionNUM,finSetNUM,COP_method,fin_area] = deal(0);
...
elseif  COM_method == 0
	...
	if  rocketCompsFromFile == false
        ...
        while keep_adding == true
            ...
            component_position(componentCount)  = (input('Enter the distance of the component from the nose tip: '))*10^-metricMass;
    ...
    elseif rocketCompsFromFile == true
        ...
        disp([  'Your rocket components will be listed below,'...
                'with their moment of inertia about the COM at launch:'])
	...
	centreOfMass_noProp = sum(~component_isProp.*component_mass.*component_position) ...
                        / sum(~component_isProp.*component_mass);
	fprintf('%s\n', ['The COM at launch is located ',num2str(centreOfMass*10^3),' mm from the nose tip.'],...
                    ['The COM at propellant depletion is located ',num2str(centreOfMass_noProp*10^3),' mm from the nose tip.'])
      
	MOInertia           = sum(cell2mat(component_MOI));
	MOInertia_noProp    = sum(cell2mat(component_MOI).*~component_isProp);
	fprintf('%s\n', ['The rocket''s MOI at launch is ',num2str(MOInertia),' kg.m^2'],...
                    ['The rocket''s MOI at propellant depletion is ',num2str(MOInertia_noProp),' kg.m^2'])
    ...
    for massFlow_count = 1:numOfStages
        ...
        %
        if  motorOFratio == false
            phase_massFlow(3*massFlow_count-1) = sum(component_mass.*component_isProp) / burnout(1);
        end % fix for inaccurate mass flow rate (inputs data not matching rocket components data)
    end
...
fprintf('%s\n','There are three phases of flight for which the effects of wind (dynamic stability) can be simulated:',...
    ...
    '2. The unpowered ascent phase (from burnout till apogee)',...
    '3. The unpowered descent phase (from apogee till ground impact)',...
    'Simulate dynamic stability for...')
    dynStabPhases = cell(1,3);
for dynStabCount = 1:3
    switch dynStabCount
        case 1
            dynStabPhase = '...the powered ascent phase';
        case 2
            dynStabPhase = '...the unpowered ascent phase';
        case 3
            dynStabPhase = '...the unpowered descent phase';
    end
    dynStabPhases{dynStabCount} = max(strcmp((input([dynStabPhase,'? [Y/N]: '],'s')),yes));
end
%}
...
sim_thrustAngle = zeros(1,round(expFlightDur/timeStep));

control.Moveable_Fins = 1; control.Gimbaled_Thrust = 2;
control.Vernier_Rocket = 3; control.Thrust_Vane = 4;

    
fprintf('%s\n','As of alpha 1.03, this program can simulate certain rocket control mechanisms.',...
...
while simCounter < numOfSteps
    ...
    if  (simCounter > 1) && (sim_altitude2(simCounter) - sim_altitude2(simCounter-1) < 0) ...
        ...
        if ((failureMode == 7) || (failureMode == 8))
            disp(['Failure mode no.',num2str(failureMode),...
                  ' experienced at T = ',num2str(sim_time(simCounter)),'s.'])
            break
        else
            descentPhase = true;
        end
	...
	if  (simCounter > 1) && (sim_altitude2(simCounter-1) > ...
                            (towerHeight*sin(pitchAngle*(pi/180))))
        offLaunchRails = true;
	end
    
    
    if  sim_time(simCounter) <= burnout(1)
        simPhase = 1;
    else
        if      descentPhase == false
                simPhase = 2;
        elseif  descentPhase == true
                simPhase = 3;
        end
    end
	...
	if (failureMode == 4 || failureMode == 6) && ...
        ...
        if  (sim_time(simCounter)-timeStep) < motorFailTime
        ...
        sim_massFlow(simCounter) = phase_massFlow(phase(sim_flightPhase(simCounter+1))); %%%DEV:NB%%% 
        % override for thrust curves (variable thrust -> variable mass flow)?
	...
	%{
	if  dynStabPhases{simPhase} == false && simCounter > 1
        if  sim_windVelocity(simCounter-1) == 0 && exist('saveWind','var') == 2
            sim_windVelocity(simCounter-1) = saveWind;
        end
            saveWind = sim_windVelocity(simCounter);
            sim_windVelocity(simCounter) = 0;
	end
    %}
        for massCount = 1:length(sim_componentMass)
            ...
            elseif  (component_isProp(massCount) == true) && (motorOFratio == true)
                if      component_mass(massCount) == propellantMass
                    ...
                    sim_propellantMass(simCounter) = sim_componentMass(massCount);
                    if  (sim_massFlow(simCounter) == 0) && (sim_propellantMass(simCounter) > 0)
                        leftoverMass = sim_propellantMass(simCounter);
                        sim_componentMass(massCount) = 0;
                        sim_propellantMass(simCounter) = 0;
                    end
                ...
                elseif  component_mass(massCount) == fuelMass
                    ...
                    if  leftoverMass ~= 0
                        sim_componentMass(massCount) = sim_componentMass(massCount) - leftoverMass * ...
                        (1/(stage_motorOFratio(firingCounter)+1));
                        leftoverMass = 0;
                    end
                    sim_fuelMass(simCounter) = sim_componentMass(massCount);
    ...
    if  simCounter > 1
        if  (sim_altitude1(simCounter) < (towerHeight*sin(pitchAngle*(pi/180)))) && ...
                (sim_time(simCounter) <= burnout(end)) && (offLaunchRails == false)
            
            sim_angle1DEG(simCounter) = pitchAngle;
            %sim_angle1DEG_perp(simCounter) = yawAngle;
            sim_pitchAngleRAD(simCounter) = pitchAngle*(pi/180);
        else
            sim_angle1DEG(simCounter) = sim_angle2DEG(simCounter-1);
            %sim_angle1DEG_perp(simCounter) = sim_angle2DEG_perp(simCounter-1);
            
            if  (sim_time(simCounter) <= burnout(end)) || (descentPhase == true) || (wind_velocity == 0)
                sim_pitchAngleRAD(simCounter) = sim_angle1DEG(simCounter)*(pi/180);
            else
                % (placeholder) - proceed to 'iterations'
            end
        end
    else
            sim_pitchAngleRAD(simCounter) = pitchAngle*(pi/180);
    ...
    if (numOfChutes > 0) && (simCounter > 3) && (sim_time(simCounter) > burnout(end))
    ...
    if  ~(((nose_length/R_length) >= 0.2) && ((nose_length/R_length) <= 0.6))
        ...
        if  (sim_machNum(simCounter) > 0) && (setDragCoef == false)
            [sim_dragCoefO(simCounter),critRatioError] = calcParasiteDrag(  sim_altitude1(simCounter),...
                                                            ...
                                                            numOfLaunchLugs,protub_length,protub_dist,...
                                                            protub_crossArea,protub_surfArea);
...
if  (numOfChutes == 0) || ( (numOfChutes > 0) && (descentPhase == false) )
    ...
    if  sim_altitude1(simCounter) > (towerHeight*sin(pitchAngle*(pi/180)))
        sim_resultantFlow(simCounter) = sqrt( sim_velocity1(simCounter)^2 + (sim_windVelocity(simCounter)*sin(sim_angle1RAD(simCounter)))^2 );
        %resultantFlowAngle = acos( min(sim_velocity1(simCounter),resultantFlow) / max(sim_velocity1(simCounter),resultantFlow) );
        sim_resultantFlowAngle(simCounter) = atan( (sim_windVelocity(simCounter)*sin(sim_angle1RAD(simCounter))) / sim_velocity1(simCounter) );

        if  sim_time(simCounter) <= burnout
            sim_angleOfAttackRAD(simCounter) = sim_resultantFlowAngle(simCounter);
        else
            sim_angleOfAttackRAD(simCounter) = (sim_angle1RAD(simCounter) + sim_resultantFlowAngle(simCounter)) - sim_pitchAngleRAD(simCounter-1); %%%DEB:NB%%%
        end
        
            moveableFinAOA = finDeflection + sim_angleOfAttackRAD(simCounter);
        
            K = 1; % constant between 1.1 & 1.5, but 1.0 tends to be suitable
            %COPvsAOA = sim_angleOfAttackRAD(simCounter)*(  ( 4 * K * (sum(y_iArray*x_increment)-fin_area) ) ...
            %                                                / (pi * (2*nose_baseRadius)^2)  );
            COPvsAOA = sim_resultantFlowAngle(simCounter)*(  ( 4 * K * (sum(y_iArray*x_increment)-fin_area) ) ...
                                                            / (pi * (2*nose_baseRadius)^2)  ); %%%DEV:NB%%%
        if  COP_selected == COP_options.Barrowman_Equations
        ...
        if  abs(sim_angleOfAttackRAD(simCounter)*(180/pi)) > 10
            sim_liftCoef(simCounter) = liftCoef(sim_angleOfAttackRAD(simCounter));
        else
            sim_liftCoef(simCounter) = (2*pi)*sim_angleOfAttackRAD(simCounter);
        end
        %
        ...
        %
            sim_windLift(simCounter) = 0.5 * sim_atmosDensity(simCounter) * sim_resultantFlow(simCounter)^2 ...
                                                                * sum(finArea) * sim_liftCoef(simCounter);
        if  controlSystem == control.Moveable_Fins
            moveableFinLift = fin_normalForce(moveableFinSet) - ...
                0.5 * sim_atmosDensity(simCounter) * sim_resultantFlow(simCounter)^2 * finArea(moveableFinSet) * moveableFinLiftCoef;
        else
            moveableFinLift = 0;
        end

            sim_angularAccel(simCounter) = ( (sim_windLift(simCounter) * (sim_COP(simCounter)-sim_COM(simCounter)))...
                        ...
                        / sim_MOI(simCounter);  % if staticMargin is positive, should rotate anticlockwise
        
        if  simCounter == 1
            sim_angularVelo(simCounter) = sim_angularAccel(simCounter)*dt;
        else
            sim_angularVelo(simCounter) = sim_angularVelo(simCounter-1) + sim_angularAccel(simCounter)*dt;
        end
        
            sim_angleDiff(simCounter) = sim_angularVelo(simCounter)*dt + 0.5*sim_angularAccel(simCounter)*dt^2;
    else
            sim_COP(simCounter) = centreOfPressure;
            sim_angleDiff(simCounter) = 0;
    end
    ...
    if  numOfChutes > 0
        ...
        for dragCount = 1:numOfChutes+1
                ...
                sim_dynPressure1(simCounter) = 0.5*sim_atmosDensity(simCounter)*( cos(sim_angleOfAttackRAD(simCounter))*(sim_velocity1(simCounter) + sim_windVelocity(simCounter)*cos(sim_angle1RAD(simCounter))) )^2; %%%DEV:NB%%%
    ...
    if ( (numOfChutes > 0) && (descentPhase == true) )
        sim_angleDiff(simCounter) = atan( (sim_windVelocity(simCounter)*sin(sim_angle1RAD(simCounter))) / sim_velocity2(simCounter) );
        sim_velocity2(simCounter) = sqrt( sim_velocity2(simCounter)^2 + (sim_windVelocity(simCounter)*sin(sim_angle1RAD(simCounter)))^2 );
    end
    
    
    if  (sim_time(simCounter) <= burnout(end)) || (descentPhase == true)
        sim_deltaAngle(simCounter) = sim_angleDiff(simCounter) + (sim_gravity(simCounter)*dt*cos(sim_angle1RAD(simCounter)))/sim_velocity2(simCounter);
    else
        sim_deltaAngle(simCounter) = (sim_gravity(simCounter)*dt*cos(sim_angle1RAD(simCounter)))/sim_velocity2(simCounter);
    end
	...
	if  simCounter > 1
        ...
        %{
        if  (sim_time(simCounter) > burnout(end)) && (descentPhase == false) && (wind_velocity ~= 0)
            sim_pitchAngleRAD(simCounter) = sim_pitchAngleRAD(simCounter-1) + sim_angleDiff(simCounter-1) + sim_deltaAngle(simCounter-1);
        else
            % (placeholder) - return to 'pre-iterations'
        end
        %}
	end
...
if  (numOfChutes == 0) || ( (numOfChutes > 0) && (descentPhase == false) )
        ...
        if  sim_altitude1(simCounter) > (towerHeight*sin(pitchAngle*(pi/180)))
            sim_resultantFlow(simCounter) = sqrt( sim_velocity2(simCounter)^2 + (sim_windVelocity(simCounter)*sin(sim_angle2RAD(simCounter)))^2 );
            %resultantFlowAngle = acos( min(sim_velocity2(simCounter),resultantFlow) / max(sim_velocity2(simCounter),resultantFlow) );
            sim_resultantFlowAngle(simCounter) = atan( (sim_windVelocity(simCounter)*sin(sim_angle2RAD(simCounter))) / sim_velocity2(simCounter) );

            if  sim_time(simCounter) <= burnout
                sim_angleOfAttackRAD(simCounter) = sim_resultantFlowAngle(simCounter);
            else
                sim_angleOfAttackRAD(simCounter) = sim_angle1RAD(simCounter) + sim_resultantFlowAngle(simCounter) - sim_pitchAngleRAD(simCounter-1); %%%DEV:NB%%%
            end
        
                moveableFinAOA = finDeflection + sim_angleOfAttackRAD(simCounter);
            
            %COPvsAOA = sim_angleOfAttackRAD(simCounter)*(  ( 4 * K * (sum(y_iArray*x_increment)-fin_area) ) / (pi * (2*nose_baseRadius)^2)  );
            COPvsAOA = sim_resultantFlowAngle(simCounter)*(  ( 4 * K * (sum(y_iArray*x_increment)-fin_area) ) / (pi * (2*nose_baseRadius)^2)  ); %%%DEV:NB%%%
            if  COP_selected == COP_options.Barrowman_Equations
                sim_COP(simCounter) = ( COP_numerator   + COPvsAOA*centreOfPlanformArea )...
                                    / ( COP_denominator + COPvsAOA );
            else
            ...
            sim_windLift(simCounter) = 0.5 * sim_atmosDensity(simCounter) * sim_resultantFlow(simCounter)^2 * sum(finArea) * sim_liftCoef(simCounter);
        ...
        if numOfChutes > 0
            ...
            for dragCount = 1:numOfChutes+1
                ...
                sim_dynPressure2(simCounter) = 0.5*sim_atmosDensity(simCounter)*( cos(sim_angleOfAttackRAD(simCounter))*(sim_velocity2(simCounter) + sim_windVelocity(simCounter)*cos(sim_angle2RAD(simCounter))) )^2; %%%DEV:NB%%%
        ...
        if iterationCount == 5
            ...
            if  (sim_altitude1(simCounter) < (towerHeight*sin(pitchAngle*(pi/180)))) && (sim_time(simCounter) <= burnout(end))
                ...
                sim_angle2RAD(simCounter) = sim_angle1RAD(simCounter) - sim_deltaAngle(simCounter);
                if  (sim_time(simCounter) > burnout(end)) && (descentPhase == false) && (wind_velocity ~= 0)
                    pitchChange = ( sim_gravity(simCounter) * dt * ...
                                    cos( sim_angle1RAD(simCounter) - sim_angleDiff(simCounter) + ...
                                        (sim_gravity(simCounter)*dt*cos(sim_angle1RAD(simCounter)))...
                                    / sim_velocity2(simCounter) )  )... 
                                / sim_velocity2(simCounter); % fix for dynamic stability - separating the FPA from pitch angle
                    sim_pitchAngleRAD(simCounter) = sim_pitchAngleRAD(simCounter-1) + pitchChange; %%%DEV:NB%%%
                else
                    % (placeholder) - return to 'pre-iterations'
                end
            end
    ...
    %if  (sim_time(simCounter) <= burnout(end)) || (descentPhase == true)
    %    sim_deltaAngle(simCounter) = sim_angleDiff(simCounter) + ( sim_gravity(simCounter) * dt * cos(sim_angle2RAD(simCounter)) ) / sim_velocity2(simCounter);
    ...
    sim_deltaDisplace(simCounter)   = (dt*sim_velocity1(simCounter))+( 0.5*(dt^2)*((sim_accel1(simCounter)+sim_accel2(simCounter))/2) );
    ...
    %% Debugging %%%DEV:NB%%%
    %
    if  (sim_deltaDisplace(simCounter) < 0) && (sim_altitude1(simCounter) ~= 0) % ~isreal(sim_...(simCounter)) || abs() == Inf || isnan() == true
        %error('Something went wrong...')
        debugMSG = 'Error occured whilst calculating rocket motion during descent. Concluding flight simulation.';
        warning(debugMSG)
        break
    end
    %{
	if  (simCounter>1) && (sim_time(simCounter)<burnout(end)) && (sim_altitude2(simCounter)<sim_altitude2(simCounter-1))
        warning('Severe weathercocking experienced (rocket pointing towards ground during burn). Flight simulation terminating.')
        break
	end
	%}
...
sim_pitchAngleDEG = sim_pitchAngleRAD*(180/pi);
sim_angleDiffDEG = sim_angleDiff*(180/pi);
sim_angleOfAttackDEG = sim_angleOfAttackRAD*(180/pi);

if  max(sim_angleOfAttackRAD*(180/pi)) > 125
...
for simEndCount = length(sim_altitude1):-1:2
    ...
    [warnMSG,~] = lastwarn;
    if  (exist('debugMSG','var')) && (strcmp(warnMSG,debugMSG) == 1)
        simEndIndex = simEndIndex - 1;
    end
    ...
    sim_angleOfAttackRAD   = sim_angleOfAttackRAD(1:simEndIndex);
    ...
    sim_pitchAngleRAD   = sim_pitchAngleRAD(1:simEndIndex);
    sim_angleDiff       = sim_angleDiff(1:simEndIndex);
    sim_angularVelo     = sim_angularVelo(1:simEndIndex);
    sim_angularAccel    = sim_angularAccel(1:simEndIndex);
...
totalResults = [sim_time.',sim_thrust.',sim_massFlow.',sim_mass2.',sim_gravity.', ...
                ...
                sim_potEnergy.',sim_kinEnergy.',sim_totalEnergy.',sim_Reynolds.',sim_stability.',sim_pitchAngleRAD'];
...
resultNames = { 'Time_(s)','Thrust_(N)','Mass_Flow_(kg/s)','Mass_(kg)','Gravity_(m/s/s)', ...
                ...
                'Potential_Energy_(MJ)','Kinetic_Energy_(MJ)','Total_Energy_(MJ)','Reynolds_Number','Stability Calibers','Pitch_Angle_(rads)'};

excelCellRange = {  'A2:A','B2:B','C2:C','D2:D','E2:E','F2:F','G2:G','H2:H', ...
                    'I2:I','J2:J','K2:K','L2:L','M2:M','N2:N','O2:O','P2:P', ...
                    'Q2:Q','R2:R','S2:S','T2:T','U2:U','V2:V','W2:W','X2:X' };
    ...
    for resultsCount = 1:(size(totalResults,2))
        try
            xlswrite( ['All_Results_(',num2str(simRunNum),')'], totalResults((1:end),resultsCount), [cell2mat(excelCellRange(resultsCount)),num2str(length(sim_time))] );
                ...
                if  strcmp(ME.identifier,'MATLAB:xlswrite:SelectDataRange')
                    warning('Too much data to write to a spreadsheet. Aborting this process.')
                    break
                end
        end
    end
...
makeGraph('Line',simRunNum,'Velocity Vector Angle','Time (s)','Flight Path Angle (deg)',sim_time,sim_angle2DEG)
makeGraph('Line',simRunNum,'Pitch Angle','Time (s)','Pitch angle (deg)',sim_time(1:ApoIndex),sim_pitchAngleRAD(1:ApoIndex)*(180/pi))
...
if  (wind_velocity ~= 0)
    ...
    makeGraph('Line',simRunNum,'Angle of Attack (up to apogee)','Time (s)','Angle (deg)',sim_time(1:ApoIndex),sim_angleOfAttackRAD(1:ApoIndex))
    makeGraph('Scatter',simRunNum,'Lift Coefficient vs. Angle of Attack','Angle of Attack (deg)','Lift Coefficient',sim_angleOfAttackRAD*(180/pi),sim_liftCoef)
	...
	makeGraph('Line',simRunNum,'FPA vs. Pitch','Time (s)','Angle (Deg)','FPA','Pitch','Burnout', ...
          sim_time(1:ApoIndex),sim_angle2DEG(1:ApoIndex),sim_time(1:ApoIndex),sim_pitchAngleDEG(1:ApoIndex),[sim_time(max(find(sim_time<=burnout))),sim_time(max(find(sim_time<=burnout)))],[pitchAngle+45,pitchAngle-45])
end
...
if  (wind_velocity ~= 0) && (windEffect_method == true)
	...
	makeGraph('Line',simRunNum,'Randomized Wind Speed Data','Simulation time (s)','Wind speed (m/s)',...
      ...
      % built-in matlab function fails under certain circumstances
      ...
      for   countA = 1:length(windSpeedAxis)
            ...
            probDens_Fnct = probDens_Fnct*(1/(round(max(probDens_Fnct)/max(probDens_Data))));
...
elseif (wind_velocity ~= 0) && (windEffect_method == false)
    ...
    clear sigma
    while exist('sigma','var') == 0
        sigma = input('Enter a value for the turbulence intensity (per-cent): ');
        if  ~isscalar(sigma) || (sigma < 0 || sigma > 100)
            disp('Invalid input. Turbulence intensity must be between 0 and 100 per-cent.')
            clear sigma
        else
            sigma = sigma/100;
        end
...
set(trajFigure, 'Units', 'normalized', 'Position', [0.01,0.15,0.975,0.75])
	...
	if  wind_velocity ~= 0
        if  (2*max(sim_range2)) > max(sim_altitude2)
            axis([  -1* (max(sim_range2)),...
                        (max(sim_range2)),...
                0, 1.05*(max(sim_range2))*2 ])
        else
            axis([  -1* (max(sim_altitude2)/2),...
                        (max(sim_altitude2)/2),...
                0, 1.05*(max(sim_altitude2))    ])
        end
    else
            axis( [ 0,...
...
while	animCounter <= length(sim_time)
        ...
        %
        if      animCounter == 1 
                addpoints(anim_line,sim_range2(animCounter),sim_altitude2(animCounter));
                drawnow
            ...
            if	( time_past <= timeStep ) && ( time_past > (1-errorMargin)*timeStep )
                ...
                if          ( (failureMode == 4 || failureMode == 6) && (sim_time(animCounter) < motorFailTime) ) ...
                        ||  ( sim_time(animCounter) < burnout(length(burnout)) )
                            ...
                            addpoints(burnLine,sim_range2(animCounter),sim_altitude2(animCounter))
                
                elseif  ( (failureMode ~= 2) && (failureMode ~= 5) ) && ...
                        ( (numOfChutes > 0) && (sim_time(animCounter) > deployTime(1)) )
                    
                        %addpoints(recoverLine,sim_range2(animCounter),sim_altitude2(animCounter))
                        break
                else
                        addpoints(anim_line,sim_range2(animCounter),sim_altitude2(animCounter));
                end
...
%%
terminateAnim = false;
filename = [rocketName,' trajectory_animation_(',num2str(simRunNum),').gif'];
fprintf('%s\n','Another trajectory animation will now play which features a visualisation',...
        'of the fuel and/or propellant depleting over time. The screen will flicker,',...
        'so you may wish to look away until it''s complete.',...
        ['The animation will be saved as a GIF file named ',filename])
    for countdown = 5:-1:1
        disp(['Animation will begin playing in ',num2str(countdown),' seconds'])
        tic; while toc < 1; end
    end
        disp('Animation will now play.')
figure('DoubleBuffer','On'); % Prevents (or reduces?) flickering

    for animCounter = 1:length(sim_time)
        tic; while toc < timeStep; end
        clf
                    fuelLevel       = sim_fuelMass(animCounter)/sim_fuelMass(1); % change value
                    oxidiserLevel   = sim_propellantMass(animCounter)/sim_propellantMass(1); % change value
                    
                subplot('Position',[0,0.5,0.25,0.25])
                    fill([0,1,1,0,0],[0,0,fuelLevel,fuelLevel,0],'g')
                    title(['Fuel: ',num2str(round(100*fuelLevel)),'%'])
                    set(gca,'XTick',[]);
                    axis square
                    axis([0,1,0,1])
                    drawnow
                    
                subplot('Position',[0.25,0.5,0.25,0.25])
                    fill([0,1,1,0,0],[0,0,oxidiserLevel,oxidiserLevel,0],'c')
                    title(['Oxidiser: ',num2str(round(100*oxidiserLevel)),'%'])
                    set(gca,'XTick',[]);
                    axis square
                    axis([0,1,0,1])
                    drawnow
                ...
                subplot('Position',[0.5,0.25,0.5,0.5])
                    ...
                    %plot(sim_range2(1:animCounter),sim_altitude2(1:animCounter))
                    if          ( (failureMode == 4 || failureMode == 6) && (sim_time(animCounter) < motorFailTime) ) ...
                            ||  ( sim_time(animCounter) < burnout(length(burnout)) )

                                %addpoints(burnLine,sim_range2(animCounter),sim_altitude2(animCounter))
                                plot(sim_range2(1:animCounter),sim_altitude2(1:animCounter),'Color','r','Marker','*')
                                burnoutPoint = animCounter;
                                legend('Engine ON')
                    ...
                    elseif  ( (failureMode ~= 2) && (failureMode ~= 5) ) && ...
                            ( (numOfChutes > 0) && (sim_time(animCounter) > deployTime(1)) )
                                ...
                                %addpoints(recoverLine,sim_range2(animCounter),sim_altitude2(animCounter))
                                plot(sim_range2(animCounter:end),sim_altitude2(animCounter:end),'Color','b','Marker','o')
                                terminateAnim = true;
                                legend('Chute(s) Deployed')
                                hold on
                                plot(sim_range2(burnoutPoint+1:animCounter-1),sim_altitude2(burnoutPoint+1:animCounter-1),'Color','k','Marker','.')
                                hold on
                                plot(sim_range2(1:burnoutPoint),sim_altitude2(1:burnoutPoint),'Color','r','Marker','*')
                    else
                                %addpoints(anim_line,sim_range2(animCounter),sim_altitude2(animCounter));
                                plot(sim_range2(burnoutPoint+1:animCounter),sim_altitude2(burnoutPoint+1:animCounter),'Color','k','Marker','.')
                                hold on
                                plot(sim_range2(1:burnoutPoint),sim_altitude2(1:burnoutPoint),'Color','r','Marker','*')
                                legend('Engine OFF')
                    end
                    
                    title(['Simulation time: ',num2str(sim_time(animCounter)),' seconds'])
                    xlabel('Range (Lateral Distance) (m)')
                    ylabel('Altitude (m)')
                    axis square
                ...
                [indexedImage,colourMap] = rgb2ind((frame2im(frame)),256);
                
                if      animCounter == 1
                        imwrite(indexedImage,colourMap,filename,'gif','LoopCount',Inf,...
                            'DelayTime',1);
                elseif  (animCounter == length(sim_time)) || (terminateAnim == true)
...
while	fpaCounter <= length(sim_time)
        ...
        if      (rem(sim_time(fpaCounter),1) == 0) && (tCounter(1) ~= tCounter(2))
                ...
                if  ( ~(numOfChutes > 0 && sim_time(fpaCounter) > deployTime(1)) || (failureMode == 2) || (failureMode == 5) ) == true
            ...
            if	( time_past <= timeStep ) && ( time_past > (1-errorMargin)*timeStep )
                ...
                if  (numOfChutes > 0) && (sim_time(fpaCounter) > deployTime(1)) && ( (failureMode ~= 2) && (failureMode ~= 5) )
                    
                    TransformMatrix = [ cos(sim_pitchAngleRAD(fpaCounter)+pi),-sin(sim_pitchAngleRAD(fpaCounter)+pi); ...
                                        sin(sim_pitchAngleRAD(fpaCounter)+pi),cos(sim_pitchAngleRAD(fpaCounter)+pi) ];
                    ...
                    TransformMatrix = [ cos(sim_pitchAngleRAD(fpaCounter)),-sin(sim_pitchAngleRAD(fpaCounter)); ...
                                        sin(sim_pitchAngleRAD(fpaCounter)),cos(sim_pitchAngleRAD(fpaCounter)) ];
                end
                ...
                if (numOfChutes > 0) && ...
                   ( (failureMode ~= 2) && (failureMode ~= 5) ) == true && ...
...
filename = [rocketName,' stability_animation_(',num2str(simRunNum),').gif'];
...
terminateAnim = false;
filename = [rocketName,' all-in-one_animation_(',num2str(simRunNum),').gif'];
    for countdown = 5:-1:1
        disp(['Animation will begin playing in ',num2str(countdown),' seconds'])
        tic; while toc < 1; end
    end
...
set(gcf,'units','normalized','position',[0 0 1 1]);
    ...
    for allInOneCounter = 1:length(sim_time)
        tic; while toc < timeStep; end
        clf
                    fuelLevel       = sim_fuelMass(allInOneCounter)/sim_fuelMass(1); % change value
                    oxidiserLevel   = sim_propellantMass(allInOneCounter)/sim_propellantMass(1); % change value
                ...
                subplot(4,4,[7,8,11,12])
                    ...
                    %plot(sim_range2(1:animCounter),sim_altitude2(1:animCounter))
                    if          ( (failureMode == 4 || failureMode == 6) && (sim_time(allInOneCounter) < motorFailTime) ) ...
                            ||  ( sim_time(allInOneCounter) < burnout(length(burnout)) )

                                %addpoints(burnLine,sim_range2(animCounter),sim_altitude2(animCounter))
                                plot(sim_range2(1:allInOneCounter),sim_altitude2(1:allInOneCounter),'Color','r','Marker','*')
                                burnoutPoint = allInOneCounter;
                                legend('Engine ON')

                    elseif  ( (failureMode ~= 2) && (failureMode ~= 5) ) && ...
                            ( (numOfChutes > 0) && (sim_time(allInOneCounter) > deployTime(1)) )

                                %addpoints(recoverLine,sim_range2(animCounter),sim_altitude2(animCounter))
                                plot(sim_range2(allInOneCounter:end),sim_altitude2(allInOneCounter:end),'Color','b','Marker','o')
                                terminateAnim = true;
                                legend('Chute(s) Deployed')
                                hold on
                                plot(sim_range2(burnoutPoint+1:allInOneCounter-1),sim_altitude2(burnoutPoint+1:allInOneCounter-1),'Color','k','Marker','.')
                                hold on
                                plot(sim_range2(1:burnoutPoint),sim_altitude2(1:burnoutPoint),'Color','r','Marker','*')
                    else
                                %addpoints(anim_line,sim_range2(animCounter),sim_altitude2(animCounter));
                                plot(sim_range2(burnoutPoint+1:allInOneCounter),sim_altitude2(burnoutPoint+1:allInOneCounter),'Color','k','Marker','.')
                                hold on
                                plot(sim_range2(1:burnoutPoint),sim_altitude2(1:burnoutPoint),'Color','r','Marker','*')
                                legend('Engine OFF')
                    end
                    
                    title(['Simulation time: ',num2str(sim_time(allInOneCounter)),' seconds'])
                ...
                subplot(4,4,15)
                    if sim_machNum(allInOneCounter) > 1
                        machLevel = (sim_machNum(allInOneCounter)-1)/(max(sim_machNum)-1);
                    else
                        machLevel = sim_machNum(allInOneCounter);
                    end
                    fill([0.25,0.75,0.75,0.25,0.25],[0,0,machLevel,machLevel,0],[1,(1-machLevel),0])
                    title(['Mach No.: [',num2str(round(sim_machNum(allInOneCounter),2)),']'])
                    set(gca,'XTick',[]);
                    axis square
                    xlabel('[velocity] / [local speed of sound]')
                    if sim_machNum(allInOneCounter) > 1
                        ylabel('SUPERSONIC')
                        set(gca,'YTick',[1,1+(ceil(max(sim_machNum))-1)/2,ceil(max(sim_machNum))]);
                        hold on
                        axis([0,1,0,ceil(max(sim_machNum))])
                        fill([0,1,1,0,0],[0,0,sim_machNum(allInOneCounter),sim_machNum(allInOneCounter),0],[(1-machLevel),0,0])
                    else
                        axis([0,1,0,1])
                        ylabel('SUBSONIC')
                ...
                subplot(4,4,16)
                    circleAngles = linspace(0,2*pi);
                    circlePoints = [cos(circleAngles);sin(circleAngles)];
                    plot(circlePoints(1,:),circlePoints(2,:),'k'); hold on
                    plot(0.9*circlePoints(1,1:63),0.9*circlePoints(2,1:63),'Color','k','LineWidth',2); hold on
                    plot(0.9*circlePoints(1,88:end),0.9*circlePoints(2,88:end),'Color','r','LineWidth',2); hold on
                    axis square
                    set(gca,'XTick',[]);
                    set(gca,'YTick',[]);
                    sim_GeeForce = sim_accel2./sim_gravity;
                    GeeForceAngle = (5/4)*pi - ((3/2)*pi * abs(sim_GeeForce(allInOneCounter)/ceil(max(sim_GeeForce))) );
                    plot([0,0.8*cos(GeeForceAngle)],[0,0.8*sin(GeeForceAngle)],'Color','r','LineWidth',2)
                    title(['Gee Force: [',num2str(round(abs(sim_GeeForce(allInOneCounter)),2)),']'])
                    xlabel('[absolute acceleration] / [local gravity]')
                    text(0.75*cos((5/4)*pi),0.75*sin((5/4)*pi),num2str(0))
                    text(0.75*cos((2/4)*pi),0.75*sin((2/4)*pi),num2str(ceil(max(sim_GeeForce))/2))
                    text(0.75*cos((7/4)*pi),0.75*sin((7/4)*pi),num2str(ceil(max(sim_GeeForce))))
                ...
                subplot(4,4,[5,6,9,10])
                    ...
                    if  (numOfChutes > 0) && (sim_time(allInOneCounter) > deployTime(1)) && ( (failureMode ~= 2) && (failureMode ~= 5) )

                        TransformMatrix = [ cos(sim_pitchAngleRAD(allInOneCounter)+pi),-sin(sim_pitchAngleRAD(allInOneCounter)+pi); ...
                                            sin(sim_pitchAngleRAD(allInOneCounter)+pi),cos(sim_pitchAngleRAD(allInOneCounter)+pi) ];

                        xChute = linspace(-chuteDiameter(1)/2,chuteDiameter(1)/2,100);
                        yChute = sqrt((chuteDiameter(1)/2)^2 - (xChute).^2);
                        TxyChute{1} = TransformMatrix * [(xChute(end/2:end)+sim_COM(allInOneCounter));yChute(end/2:end)];
                        TxyChute{2} = TransformMatrix * [(xChute(end/2:end)+sim_COM(allInOneCounter));-yChute(end/2:end)];
                        TxyCords{1} = TransformMatrix * [(xChute(1:end/2)+sim_COM(allInOneCounter));fliplr(xChute(1:end/2))];
                        TxyCords{2} = TransformMatrix * [(xChute(1:end/2)+sim_COM(allInOneCounter));-fliplr(xChute(1:end/2))];

                        plot(TxyChute{1}(1,:),TxyChute{1}(2,:),'-b',TxyChute{2}(1,:),TxyChute{2}(2,:),'-b',...
                             TxyCords{1}(1,:),TxyCords{1}(2,:),'-.b',TxyCords{2}(1,:),TxyCords{2}(2,:),'-.b'); hold on

                    else
                        TransformMatrix = [ cos(sim_pitchAngleRAD(allInOneCounter)),-sin(sim_pitchAngleRAD(allInOneCounter)); ...
                                            sin(sim_pitchAngleRAD(allInOneCounter)),cos(sim_pitchAngleRAD(allInOneCounter)) ];
                    end

                    Ty_i            = TransformMatrix * [fliplr(x_iArray)-(R_length-sim_COM(allInOneCounter));y_i];

                    Ty_i_projected	= TransformMatrix * [fliplr(x_iArray)-(R_length-sim_COM(allInOneCounter));-y_i_projected];

                    TxyEnd          = TransformMatrix * [0*xEnd-(R_length-sim_COM(allInOneCounter));yEnd];
                    ...
                    if  sim_time(allInOneCounter) < burnout(length(burnout))
                        ...
                        TxyEnd_firing = TransformMatrix * [ xEnd_firing-(R_length-sim_COM(allInOneCounter)) ; yEnd_firing ];
                        distToOriginX = sum(TxyEnd_firing(1,2:3))/2;
                        distToOriginY = sum(TxyEnd_firing(2,2:3))/2;
                        for gimbalCount = 1:5
                            TxyEnd_firing(:,gimbalCount) = TxyEnd_firing(:,gimbalCount) - [distToOriginX;distToOriginY]; %translate to origin
                            TxyEnd_firing(:,gimbalCount) = [ cos(sim_thrustAngle(allInOneCounter)),-sin(sim_thrustAngle(allInOneCounter));...
                                                              sin(sim_thrustAngle(allInOneCounter)),cos(sim_thrustAngle(allInOneCounter)) ]...
                                                         * TxyEnd_firing(:,gimbalCount); % rotate about origin
                            TxyEnd_firing(:,gimbalCount) = TxyEnd_firing(:,gimbalCount) + [distToOriginX;distToOriginY]; % translate back
                        end
                    ...
                    Transformed_COP = TransformMatrix*[(sim_COM(allInOneCounter)-sim_COP(allInOneCounter));0];
                    ...
                    xticklabel = linspace(  round( sim_range1(allInOneCounter)-(sim_COM(allInOneCounter)+centreOfPressure)/2, 2 ),...
                                            round( sim_range1(allInOneCounter)+(sim_COM(allInOneCounter)+centreOfPressure)/2, 2 ), 5);

                    yticklabel = linspace(  round( sim_altitude1(allInOneCounter)-(sim_COM(allInOneCounter)+centreOfPressure)/2, 2 ),...
                                            round( sim_altitude1(allInOneCounter)+(sim_COM(allInOneCounter)+centreOfPressure)/2, 2 ), 5);

                    xyTick = [ round( -(sim_COM(allInOneCounter)+centreOfPressure)/2, 2), round( -(sim_COM(allInOneCounter)+centreOfPressure)/4, 2 ), 0,...
                               round( (sim_COM(allInOneCounter)+centreOfPressure)/4, 2 ), round( (sim_COM(allInOneCounter)+centreOfPressure)/2, 2 )];

                    set(gca,'XTick',xyTick)
                    set(gca,'XTickLabel',xticklabel);
                    %set(gca,'XTickLabelRotation',45)
                    %xlabel('Range (m)')
                    XLabel1 = xlabel('Range (m)');
                    set(XLabel1, 'Units', 'Normalized', 'Position', [1.2, 0.05, 0]);
                    ...
                    axis( [ -(centreOfMass+centreOfPressure)/2, (centreOfMass+centreOfPressure)/2,...
                            -(centreOfMass+centreOfPressure)/2, (centreOfMass+centreOfPressure)/2 ] )
                    axis square
                    legend([COMpoint,COPpoint],{'COM','COP'})
                ...
                subplot(4,4,[1,2])
                    timeRange1 = 5;
                    if      sim_time(allInOneCounter) <= timeRange1
                            plot(   sim_time( 1 : (allInOneCounter+(timeRange1/timeStep)) ),...
                                    sim_COM( 1 : (allInOneCounter+(timeRange1/timeStep)) ) )
                            hold on
                            plot(   sim_time( 1 : (allInOneCounter+(timeRange1/timeStep)) ),...
                                    sim_COP( 1 : (allInOneCounter+(timeRange1/timeStep)) ) )
                            
                    elseif  sim_time(allInOneCounter) > (sim_time(end)-timeRange1)
                            plot(   sim_time( (allInOneCounter-(timeRange1/timeStep)) : end ),...
                                    sim_COM( (allInOneCounter-(timeRange1/timeStep)) : end ) )
                            hold on
                            plot(   sim_time( (allInOneCounter-(timeRange1/timeStep)) : end ),...
                                    sim_COP( (allInOneCounter-(timeRange1/timeStep)) : end ) )
                    else
                            plot(   sim_time( (allInOneCounter-(timeRange1/timeStep)) : (allInOneCounter+(timeRange1/timeStep)) ),...
                                    sim_COM( (allInOneCounter-(timeRange1/timeStep)) : (allInOneCounter+(timeRange1/timeStep)) ) )
                            hold on
                            plot(   sim_time( (allInOneCounter-(timeRange1/timeStep)) : (allInOneCounter+(timeRange1/timeStep)) ),...
                                    sim_COP( (allInOneCounter-(timeRange1/timeStep)) : (allInOneCounter+(timeRange1/timeStep)) ) )
                            xlim([sim_time(allInOneCounter-(timeRange1/timeStep)),sim_time(allInOneCounter+(timeRange1/timeStep))])
                    end
                    xlim( [sim_time(allInOneCounter)-timeRange1, sim_time(allInOneCounter)+timeRange1] )
                    ylim( [min([min(sim_COM(1:ApoIndex)),min(sim_COP(1:ApoIndex))]), max([max(sim_COM(1:ApoIndex)),max(sim_COP(1:ApoIndex))])] )
                    title('COM vs. COP (distance from ref.)')
                    %xlabel('Time (s)')
                    XLabel2 = xlabel('Time (s)');
                    set(XLabel2, 'Units', 'Normalized', 'Position', [0.95, -0.15, 0]);
                    ylabel('Distance from ref. (m)')
                    legend('Centre of Mass (COM)','Centre of Pressure (COP)')
                    hold on
                    plot(   [sim_time(allInOneCounter),sim_time(allInOneCounter)] , ...
                            [min([min(sim_COM),min(sim_COP)]), ...
                             max([max(sim_COM),max(sim_COP)])], 'Color','k','LineWidth',2  )
                    drawnow
                    
                    
                subplot(4,4,[13,14])
                    timeRange2 = 5;
                    if      sim_time(allInOneCounter) <= timeRange2
                            plot( sim_angle2DEG( 1 : (allInOneCounter+(timeRange2/timeStep)) ) )
                            hold on
                            plot( sim_pitchAngleDEG( 1 : (allInOneCounter+(timeRange2/timeStep)) ) )
                    
                    elseif  sim_time(allInOneCounter) > (sim_time(end)-timeRange2)
                            plot( sim_angle2DEG( (allInOneCounter-(timeRange2/timeStep)) : end ) )
                            hold on
                            plot( sim_pitchAngleDEG( (allInOneCounter-(timeRange2/timeStep)) : end ) )
                    else
                            plot( sim_angle2DEG( (allInOneCounter-(timeRange2/timeStep)) : (allInOneCounter+(timeRange2/timeStep)) ) )
                            hold on
                            plot( sim_pitchAngleDEG( (allInOneCounter-(timeRange2/timeStep)) : (allInOneCounter+(timeRange2/timeStep)) ) )
                    end
                    xlim( [sim_time(allInOneCounter)-timeRange2, sim_time(allInOneCounter)+timeRange2] )
                    ylim( [min([min(sim_angle2DEG(1:ApoIndex)),min(sim_pitchAngleDEG(1:ApoIndex))]), max([max(sim_angle2DEG(1:ApoIndex)),max(sim_pitchAngleDEG(1:ApoIndex))])] )
                    title('FPA vs. Pitch (anticlockwise angle from horizontal)')
                    xlabel('Time (s)')
                    ylabel('Angle (Deg.)')
                    legend('Flight Path Angle','Pitch Angle')
                    hold on
                    plot(   [sim_time(allInOneCounter),sim_time(allInOneCounter)] , ...
                            [min([min(sim_angle2DEG),min(sim_pitchAngleDEG)]), ...
                             max([max(sim_angle2DEG),max(sim_pitchAngleDEG)])], 'Color','k','LineWidth',2  )
                ...
                if      allInOneCounter == 1
                        imwrite(indexedImage,colourMap,filename,'gif','LoopCount',Inf,...
                            'DelayTime',1);
                elseif  (allInOneCounter == length(sim_time)) || (terminateAnim == true)
                        imwrite(indexedImage,colourMap,filename,'gif','WriteMode','append',...
                            'DelayTime',1);
                        break
                else
...
close all
diary off
end % un-comment this line when integrating scripts
