...
if  rocketFromFile == false
    ...
    disp('From nose to nozzle, each part one-by-one, describe the structure of your rocket using the key below: ')
...
if  (numOfChutes == 0) || ( (numOfChutes > 0) && (descentPhase == false) )
    ...
    %% Debugging %%%DEV:NB%%%
    %
    if  sim_deltaDisplace(simCounter) < 0 % ~isreal(sim_...(simCounter)) || abs() == Inf || isnan() == true
...
if  (skipDescentAnim == true) && (numOfChutes > 0) &&...
